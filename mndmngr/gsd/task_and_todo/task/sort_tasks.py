...
from mndmngr.gsd.task_and_todo.task.task_retrieval_utils import Task
...
def sort_tasks_impl(tasks: list[Task], categories: list[str], attr_sort_order: dict[str, list[str]], category_idx: int) -> list[Task]:
  ...
  for val in attrs: 
    if (val not in attr_groups):
      continue
    for task in sort_tasks_impl(attr_groups[val], categories, attr_sort_order, category_idx+1):
